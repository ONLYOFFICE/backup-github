...
while true; do
  ...
  REPOLIST_TMP=`check curl --silent -u ${GHBU_UNAME}:${GHBU_PASSWD} ${GHBU_API}/orgs/${GHBU_ORG}/repos?type=all\&page=${PAGE}\&per_page=90 -q -k | grep "\"full_name\"" | awk -F': "' '{print $2}' | sed -e 's/",//g' | sed -e 's/'${GHBU_ORG}'\///g'`
